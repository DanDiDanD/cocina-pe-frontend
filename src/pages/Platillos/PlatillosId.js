              ...
              renderItem={(item) => (
                ...
                >
                  <List.Item.Meta
                    ...
                    title={`${item.nombre ? item.nombre : ''}${item.porciones.length != 0 ? ` (${item.porciones} porciones)` : ''}`}
                    description={`${item.nombre_usuario}`}
                  />
                  <Col lg={24} className="lista-descripcion-receta">{item.descripcion ? item.descripcion : ''}</Col>
                </List.Item>
              )}
            />
            {/* <Table
